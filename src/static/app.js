                  ...
                  .map(
                    (p) =>
                      `<li class="participant-item" style="display: flex; align-items: center; gap: 6px;">
                        <span>${p}</span>
                        <span class="delete-participant" data-activity="${encodeURIComponent(name)}" data-email="${encodeURIComponent(p)}" title="Remove participant" style="cursor:pointer;color:#c62828;font-size:18px;user-select:none;">&#128465;</span>
                      </li>`
                  )
                  .join("")}
              </ul>
  ...
  signupForm.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        messageDiv.className = "success";
        signupForm.reset();
        fetchActivities(); // Actualizar la lista de actividades sin recargar la página
      } else {
        messageDiv.textContent = result.detail || "An error occurred";
        messageDiv.className = "error";
      }
